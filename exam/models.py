...
class QuestionBank(models.Model):
    ...
    a = models.CharField('A 选项', max_length=40)
    b = models.CharField('B 选项', max_length=40)
    c = models.CharField('C 选项', max_length=40)
    d = models.CharField('D 选项', max_length=40)
    answer = models.CharField('答案', choices=(('A', 'A'), ('B', 'B'), ('C', 'C'), ('D', 'D')), max_length=4)
    difficulty = models.CharField('难度', choices=(('easy', '简单'), ('middle', '中等'), ('difficult', '难')),
                                  max_length=10)
    ...
    def __str__(self):
        return '<%s:%s>' % (self.course, self.course)
...
class AdminManager(models.Model):
    ...
    role = models.CharField('角色', max_length=20, default='admin')  # 添加 role 字段并设置默认值
...
class AcademicStaff(models.Model):
    ...
    sex = models.BooleanField('性别', choices=((0, '女'), (1, '男')))
    password = models.CharField('密码', max_length=20)
    role = models.CharField('角色', max_length=20, default='academic_staff')
...
class Teacher(models.Model):
    ...
    sex = models.BooleanField('性别', choices=((0, '女'), (1, '男')))
    password = models.CharField('密码', max_length=20)
    role = models.CharField('角色', max_length=20, default='teacher')
...
class Exam(models.Model):
    title = models.CharField('考试标题', max_length=255, unique=True)
    course = models.ForeignKey(Course, on_delete=models.CASCADE, verbose_name='科目')
    start_time = models.DateTimeField('开始时间')
    end_time = models.DateTimeField('结束时间')
    duration_minutes = models.IntegerField('考试时长（分钟）')
    questions = models.ManyToManyField(QuestionBank, related_name='exam_questions')
